...
// 重新导出主要类型，方便上层调用
pub use models::{Book, Chapter, BookSource, BookProgress, Bookmark, ReplaceRule, DownloadTask, DownloadChapter};
...
mod tests {
    ...
    fn test_wal_enabled() {
        ...
        let db_path = temp_dir.path().join("test_wal.db").to_string_lossy().to_string();
        let config = DatabaseConfig { path: db_path, enable_wal: true };
    ...
    fn test_wal_disabled() {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test_no_wal.db").to_string_lossy().to_string();
        let config = DatabaseConfig { path: db_path, enable_wal: false };
        let manager = StorageManager::new(config).unwrap();
