...
impl UmdParser {
    ...
    pub fn parse_file<P: AsRef<Path>>(
        ...
        debug!(
            "UMD 文件头: v{} enc:{} ch:{} clen:{}",
            header.version, header.encrypt_info, header.chapter_count, header.content_length
        );
        ...
        if header.chapter_count > MAX_CHAPTER_COUNT {
            return Err(format!(
                "章节数量 {} 超过上限 {}",
                header.chapter_count, MAX_CHAPTER_COUNT
            ));
        }
        ...
        for (index, &offset) in chapter_offsets.iter().enumerate() {
            ...
            let (umd_title, content) =
                self.read_chapter_content(&mut file, offset, end_offset, file_len)?;
    ...
    fn read_chapter_index(
        ...
        for _ in 0..chapter_count {
            ...
            file.read_exact(&mut offset_buf).map_err(|e| {
                format!(
                    "读取章节索引失败: 预期 {} 个偏移量，实际读取失败: {}",
                    chapter_count, e
                )
            })?;
    ...
    fn parse_umd_tags(data: &[u8]) -> (Option<String>, String) {
        ...
        while pos + 1 < data.len() {
            ...
            match tag_type {
                ...
                0x83 => {
                    ...
                    pos += 4;
                    if uncomp_len > MAX_CHAPTER_SIZE {
                        break;
                    }
                    let decoder = ZlibDecoder::new(&data[pos..]);
                    let mut decompressed = Vec::with_capacity(uncomp_len.min(MAX_CHAPTER_SIZE));
                    if decoder
                        .take(MAX_CHAPTER_SIZE as u64 + 1)
                        .read_to_end(&mut decompressed)
                        .is_ok()
                        && decompressed.len() <= MAX_CHAPTER_SIZE
                    {
        ...
        if content.is_empty() {
            ...
            content = if !fallback.contains('\u{FFFD}') {
                fallback
            } else {
                fb.into_owned()
            };
