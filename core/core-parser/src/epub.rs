...
impl EpubParser {
    ...
    pub fn parse_file<P: AsRef<Path>>(
        &self,
        path: P,
    ) -> Result<(super::types::BookMetadata, Vec<super::types::Chapter>), String> {
        let path = path.as_ref();
        info!("开始解析 EPUB 文件: {:?}", path);

        let file = fs::File::open(path).map_err(|e| format!("打开文件失败: {}", e))?;
        let mut archive = ZipArchive::new(file).map_err(|e| format!("解压 EPUB 失败: {}", e))?;
        ...
        let (metadata, manifest, spine, ncx_path) =
            self.parse_content_opf(&mut archive, &content_opf_path)?;
        debug!("元数据: {:?}", metadata);
        ...
        // 3. 按 spine 顺序解析章节内容
        let chapters = self.parse_chapters(
            &mut archive,
            &content_opf_path,
            &manifest,
            &spine,
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Text(ref e)) if in_metadata => {
                    ...
                    if (current_tag.ends_with(":title") || current_tag == "title")
                        && metadata.title.is_none()
                    {
                        metadata.title = Some(text);
                    } else if (current_tag.ends_with(":creator") || current_tag == "creator")
                        && metadata.author.is_none()
                    {
                        metadata.author = Some(text);
                    } else if (current_tag.ends_with(":language") || current_tag == "language")
                        && metadata.language.is_none()
                    {
                        metadata.language = Some(text);
                    } else if (current_tag.ends_with(":identifier") || current_tag == "identifier")
                    ...
                    } else if (current_tag.ends_with(":publisher") || current_tag == "publisher")
                        && metadata.publisher.is_none()
                    {
                        metadata.publisher = Some(text);
                    } else if (current_tag.ends_with(":date") || current_tag == "date")
                        && metadata.date.is_none()
                    {
                        metadata.date = Some(text);
                    } else if (current_tag.ends_with(":rights") || current_tag == "rights")
                ...
                Ok(Event::Text(_)) => {}
                Ok(Event::End(ref e)) if e.name().as_ref() == b"metadata" => {
                    in_metadata = false;
                }
                Ok(Event::Eof) => break,
    ...
    fn parse_ncx(
        ...
        let base_path = std::path::Path::new(opf_path)
            .parent()
    ...
    fn parse_chapters(
        ...
        spine: &[String],
        ncx_titles: &std::collections::HashMap<String, String>,
    ) -> Result<Vec<super::types::Chapter>, String> {
...
/// 便捷函数：快速解析 EPUB 文件
pub fn parse_epub_file<P: AsRef<Path>>(
    path: P,
) -> Result<(super::types::BookMetadata, Vec<super::types::Chapter>), String> {
